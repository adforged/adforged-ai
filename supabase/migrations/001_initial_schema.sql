-- Initial Database Schema for AdForge AI
...
CREATE TABLE IF NOT EXISTS projects (
  ...
  -- Basic info
  name TEXT NOT NULL,
  status TEXT DEFAULT 'draft' CHECK (status IN ('draft', 'generating', 'completed', 'failed', 'processing')),
  ...
  video_url TEXT,
  thumbnail_url TEXT,
  video_job_id TEXT, -- Provider job ID for status checking
...
CREATE TABLE IF NOT EXISTS user_activity (
  ...
  -- Context
  resource_type TEXT, -- 'project', 'campaign', 'subscription'
  resource_id UUID,
...
-- Public read access for avatars, voices, templates (no RLS needed)
-- These are read-only for all authenticated users
...
-- Trigger: Automatically create user record when someone signs up via Supabase Auth
...
-- Seed some default avatars (can be expanded later)
